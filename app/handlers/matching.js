...
const notifyRelevantDrivers = async (order, ctx) => {
    try {
        ...
        if (matches.length === 0) {
            global.logger.logAction('no_drivers_to_notify', {
                orderId: order._id,
        ...
        for (const match of matches.slice(0, 5)) { // Top 5 drivers
            try {
                ...
                global.logger.logAction('driver_notified', {
                    orderId: order._id,
        ...
        global.logger.logAction('drivers_notification_completed', {
            orderId: order._id,
        ...
        global.logger.logAction('notification_error', {
            orderId: order._id,
            error: error.message
        });
...
const updateDriverExperience = async (driverId) => {
    try {
        ...
        global.logger.logAction('driver_experience_updated', {
            driverId: driver._id,
        ...
        global.logger.global.logger.logError('Error updating driver experience:', error);
    }
};
